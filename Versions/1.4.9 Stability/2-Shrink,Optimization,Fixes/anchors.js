...
function html(data,...keys){
    ...
    const states=function(list){
        ...
        get(target, key){
            target["computed"]=="function"?target.computed():""
            // MARKED-BUG:INFINITE LOOP */ typeof target[key]=="function"?(update()):""
            console.log("get")
            return target[key];
        },
        ...
        content.querySelectorAll("*").forEach((e)=>{
            ...
            attrnames.forEach((i)=>{
                if(i[0]=="$"&&typeof list[e.getAttribute(i)]){
                    ...
                    let apply=null;
                    function getResult(){
                        return typeof state[applied[0]??getted_attr]=="function"?state[applied[0]??getted_attr](apply):state[applied[0]??getted_attr]
                    }
                    //boilerPlate
                    const bp=(control,arg)=>{
                        details.childUpdates.push(control)
                        content.addEventListener(updateSeed,control)
                        document.addEventListener(updateSeed,control);
                        if(arg){
                    ...
                    case "destroy":{
                        const control=function(){
                            ...
                            if(result){
                                e.remove();
                                content.removeEventListener(updateSeed,control);
                                content.dispatchEvent(updatedEvent);
...
const diff=(first,second,confirmed)=>{
    ...
    const classDiff=(st,nd)=>{
        st.className!=nd.className?st.className=nd.className:""
    }
    ...
    attrnames.forEach((i)=>{
        if(i[0]=="$"&&typeof list[e.getAttribute(i)]!=="undefined"){
            ...
            case "model":
                ...
                //getter
                const updateModel=function(){
                    if(Anchor.valueTypeControl(e.type)||Anchor.checkedTypeControl(e.type)){
...
//export {createStore,debounce,For,nodeList,HTML,html,OnGlobalUpdate,GlobalUpdate,RegisterComponent}
