...
const html=(data,...keys)=>{
    ...
    content.children.length>1?console.warn("Anchors Warn : There must be only one parent element"):""
    ...
    const effect=(arg)=>{
        ...
        signal(arg);
        if(details.childFor.length){details.childFor.forEach((f)=>f())}
    }
    ...
    var object={isMounted,querySelector,querySelectorAll,getElementById,content:content.firstElementChild,details,unmount,effect,signal,mount,update,localUpdate,states,methods}
    return object;
}
...
const HTML=(str)=>{
    ...
    return {content,type,details,mount,unmount,states};
}
